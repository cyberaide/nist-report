...
software to allowing scripting to deploy, configure, and run such
pipelines across a range of resource providers.

To this end, we have implemented the Fingerprint Matching and Face
Detection use cases from the Big Data Working Group proposed Use Cases
...
Analytics. Additionally, we have investigated publicly source code
from GitHug \FIXME{Hyungro, is this accurate?} as well as relevant
student projects taught at Indiana University to better understand the
ecosystem of Big Data Analytics.
...
\item[Support infrastructure.] Systems such as OpenStack, Amazon EC2,
  ...
  support allocating resources for computation, storage, network. All
  listed but Comet are provide virtualized environments, the latter
  provides hypervisors running on bare-metal \FIXME{Gregor check for
    accuracy}.
...
\begin{figure}[htb]
  ...
  \centering
  \includegraphics[width=0.3\textwidth]{images/packages-heatmap}
...
\begin{table}[htb]
  ...
  \begin{tabular}{l|l|l|l|l}
    ...
    Zookeeper & framework & directory service & 3 \\ \hline
